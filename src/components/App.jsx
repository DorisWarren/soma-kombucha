...
class App extends React.Component {
  ...
  handleDeleteKombucha(remove) {
    console.log(remove)
    let newMasterKombuchaList = this.state.masterKombuchaList.slice();
    for (let i= 0; i < newMasterKombuchaList.length; i ++){
      if (remove.id === newMasterKombuchaList[i].id){
  ...
  render(){
    return (
      <div>
        <Header/>
        <Switch>
          <Route exact path='/' render={()=><KombuchaList onHandleDeleteKombucha={this.handleDeleteKombucha} KombuchaList={this.state.masterKombuchaList} />} />
